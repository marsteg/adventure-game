...
**✅ USE THIS**: The repository includes a dedicated asset generator tool at `tools/asset_generator.py` that creates game assets using AI image generation and text-to-speech audio generation.

#### Asset Requirements
- **Images**: Room backgrounds (1024x768), character sprites, items, actions, doors - **Generated by the tool**
- **NPC Dialog Audio**: Voice files for all character dialog lines (WAV format) - **Generated by the tool**
- **Background Music & SFX**: Background music, door sounds, action sound effects (WAV format) - Must be sourced separately
...
**For audio assets (NPC dialog voice files):**
```bash
# Interactive mode (select option 7: AUDIO-FILES)
python tools/asset_generator.py --interactive

# Command-line mode - generates audio for all dialog in the character's YAML file
python tools/asset_generator.py --generate-audio librarian
python tools/asset_generator.py --generate-audio wizard
python tools/asset_generator.py --generate-audio shopkeeper
```

**Audio generation workflow:**
1. Create NPC dialog YAML file in `assets/dialogs/<character>.yaml` with all dialog nodes
2. Configure character voice in `tools/voice_config.yaml` (optional, uses default if not configured)
3. Run audio generator: `python tools/asset_generator.py --generate-audio <character>`
4. Audio files are created in `assets/sounds/dialogs/` matching the naming pattern expected by the engine

**Features:**
- ✅ **FREE Unlimited Image Generation** - Uses Hugging Face API (no credit limits)
- ✅ **FREE Audio Generation** - Text-to-speech for NPC dialogs (SpeechT5, XTTS, KugelAudio)
...
**Audio Assets:**
- **Voice Acting (NPC Dialogs)**: **Use the built-in audio generator!** The asset generator can automatically generate audio for all NPC dialog lines using text-to-speech
  - Setup: See `tools/QUICKSTART.md` for audio generation setup (SpeechT5, XTTS, or KugelAudio)
  - Configure voices: Edit `tools/voice_config.yaml` to assign unique voices to each character
  - Generate: `python tools/asset_generator.py --generate-audio <character_name>`
  - The tool reads from `assets/dialogs/<character>.yaml` and generates WAV files for all dialog nodes
  - Supports character-specific voices and combines array dialogs into single audio files
...
4. **Configure Character Voices**: Update `tools/voice_config.yaml` to assign unique voices to each NPC
5. **Generate Dialog Audio**: Use the asset generator to create audio files for all NPC dialogs:
   - After creating dialog YAML files, run: `python tools/asset_generator.py --generate-audio <character_name>`
   - The tool automatically generates audio for all dialog nodes in the character's YAML file
   - See `tools/VOICE_CONFIG_GUIDE.md` for voice customization options
6. **Source Background Music/SFX**: Find or create background music, door sounds, and action sound effects
7. **Test**: Verify all assets load correctly and match your art style
8. **Iterate**: Adjust configs if assets don't match, regenerate as needed
